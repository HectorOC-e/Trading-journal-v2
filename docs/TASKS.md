...
- [ ] `dashboard/page.tsx` is ≤ 100 LOC
- [ ] Each tab is a separate file that can be read and understood independently
- [ ] `prop-firm-rules.tsx` and `chart-tooltip.tsx` have no inline data logic
- [ ] No TypeScript errors
- [ ] Dashboard functionality is visually identical after extraction
...
- [ ] `dashboard/page.tsx` calls `useDashboardStats()` and does not import `trpc` directly
- [ ] Hook returns typed `DashboardStats | undefined` during loading
- [ ] `staleTime: 60_000` prevents re-fetch on every tab switch
...
- [ ] `/dashboard`, `/aprendizaje`, `/trades`, `/cuentas` each have `loading.tsx` and `error.tsx`
- [ ] Loading state shows a skeleton that matches the page's approximate layout (not a generic spinner)
- [ ] Error state has a "Reintentar" button that calls `reset()`
- [ ] No blank white screen on simulated network error
...
- [ ] `aprendizaje/page.tsx` is ≤ 200 LOC
- [ ] Each modal file is under 300 LOC and has a single clear responsibility
- [ ] `use-resource-actions.ts` encapsulates all modal open/close state
- [ ] All existing functionality works identically after extraction
- [ ] No TypeScript errors
...
- [ ] `cuentas/page.tsx` is ≤ 200 LOC
- [ ] `AccountCard` renders identically after extraction
- [ ] Prop firm fields (phase, dd model, allowed symbols) display correctly in the edit modal
- [ ] No TypeScript errors
...
- [ ] `reviews/page.tsx` is ≤ 150 LOC
- [ ] Week date selection logic is fully encapsulated in `create-review-modal.tsx`
- [ ] All existing review CRUD functionality works
...
- [ ] `trade-service.ts` exists with the three exported functions
- [ ] `trades.ts` router calls service functions — no inline P&L or avg-entry math
- [ ] Tests pass for LONG and SHORT P&L, zero-stop R-multiple, scale-in weighted average
- [ ] `tsc --noEmit` passes
...
- [ ] All three guard functions are pure (no Prisma, no tRPC)
- [ ] Tests cover: at-limit (no violation), over-limit (violation), empty allowedSymbols (no violation), case-insensitive symbol match
- [ ] `trades.create` router calls guard functions and maps `PropFirmViolation` to `TRPCError`
...
- [ ] `computeMaxDrawdown([100, -50, 200, -150, 100])` returns `150` (peak 300, trough 150)
- [ ] `computeEquityCurve` returns chronologically sorted points starting at `initialBalance`
- [ ] `trades.dashboardStats` uses these functions instead of inline loops
- [ ] `dashboard/page.tsx` no longer contains drawdown loop logic
...
- [ ] `calcNextReviewAt(7, 1)` returns a Date 4 days from now (ceil(7/2))
- [ ] `calcNextReviewAt(7, 3)` returns a Date 7 days from now
- [ ] `calcNextReviewAt(7, 5)` returns a Date 11 days from now (round(7×1.5))
- [ ] `calcNextReviewAt(1, 1)` returns a Date 1 day from now (floor(1/2) = 0, clamped to 1)
- [ ] Router delegates to service function; no inline interval math remains in the router
...
- [ ] Same day (two reviews on 2026-05-30): streak unchanged
- [ ] Consecutive day (review on 05-29, review on 05-30): streak + 1
- [ ] Gap of 2 days (review on 05-28, review on 05-30): streak resets to 1
- [ ] First review ever (lastReviewDate = null): streak = 1
- [ ] Router `createReview` calls service function; no inline date math remains
...
- [ ] Resource with status MASTERED, overdue by `reviewInterval × 2 + 1` days: returned in decayed set
- [ ] Resource with status MASTERED, overdue by `reviewInterval × 2 - 1` days: not returned
- [ ] Resource with status MASTERED, null nextReviewAt: not returned
- [ ] Resource with status IN_REVIEW (not MASTERED): not returned
- [ ] Router `stats` calls service; `updateMany` follows if decayed set is non-empty
...
- [ ] All test cases pass with `pnpm vitest`
- [ ] 100% branch coverage on `computeClosedTradePnl`, `computeRMultiple`, `checkDailyLossLimit`, `computeMaxDrawdown`
...
- [ ] All test cases pass
- [ ] `getISOWeekKey` boundary tests match known ISO 8601 week numbers
- [ ] 100% branch coverage on the three learning services
...
- [ ] All 8 functions are pure (no Prisma, no tRPC)
- [ ] `trades.dashboardStats` router delegates entirely to service functions
- [ ] T-III-007-style tests added for `buildKpis` and `buildEquityCurve`
...
- [ ] `setups.performanceStats` procedure returns correctly computed stats for a given setup
- [ ] `TabPlaybook` uses `setups.performanceStats` instead of `useMemo`
- [ ] `resourceImpactRanking` reuses `SetupAnalyticsService.computeSetupStats` for its win-rate calculation
...
- [ ] Period selector renders in `TabPortfolio` and `TabOperador`
- [ ] Selecting "1Y" refetches and renders 52 weekly data points
- [ ] Selecting "ALL" renders monthly aggregates (1 bar per month)
