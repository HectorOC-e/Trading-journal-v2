...
| **S7** Coach v3 II (proactividad + intervención, C1): motor determinista (cascada/revenge/oversizing/DD + scoring/fatiga + override capital D14); `Intervention` (E11) + fast-path en `trades.close` (≤2s, best-effort) + `InterventionOverlay`; aceptar→crea regla. Engine unit-testeado (14) + migración live; **walkthrough live de cascada NO completado** (flakiness del modal de registro/combobox en Playwright — harness, no producto) | ✅ merged a main | #102 |
...
- **Pendiente de verificación live (S7):** completar el walkthrough cascada→overlay→aceptar-crea-regla. Bloqueó la flakiness del modal de registro (combobox de símbolo) + form de cierre en Playwright. **Lección reforzada:** crear cuenta/setup **por UI**, NO fixtures SQL (los SQL no renderizaban; ver [[feedback_qa_no_db_injection]]). Para reintentar: registrar+cerrar 3 trades perdedores el mismo día → la cascada dispara en el 3er `close`.
