...
- 🔵 **S12 EN CURSO — por partes** (primer sprint de UI pesada; viste el quant de S3–S11). Hecho: **S12a** tokens DS v3 (#108) · **S12b1** ANALIZAR = tabs Institucional(S3)+Edges(S11) en /analytics (#109) · **S12b2** PROTEGER = RiskBudgetMeter + AccountRiskPanel (S9: ruina/proyección/budget) en el detalle de cuenta de /cuentas (#110) — ambos verificados visualmente. · **S12b3** MEJORAR = SetupIntelligencePanel (S10: decay/drift/EdgeEvolutionChart) en detalle de setup + ErrorCardsPanel (S11 #42) en /playbook (#111) — verificado visualmente. **S12b COMPLETO**. · **S12c** shell de 5 superficies tras flag (#112): `lib/v3-shell-store.ts` (localStorage `tj.v3Shell`, off default) + `SURFACE_NAV` swap en Sidebar + comando ⌘K "Vista: 5 superficies". OFF=nav v2 idéntica; ON=HOY/OPERAR/MEJORAR/PROTEGER/ANALIZAR. Verificado ambos estados. Sin cambios de ruta (reversible). **Pendiente:** **S12d** intervención global (montar InterventionOverlay global) + onboarding día-1. **Diferido en S12c:** migración real de rutas (crear /hoy,/operar… absorbiendo Dashboard/Notif/Mercados/Etiquetas) → progresivo cuando el shell se valide en uso. **Diferido:** transfer #31 + SRS #45 (resource-centric → /aprendizaje, no /playbook; backend S11 ya existe). Componentes reutilizables: `components/analytics/{r-distribution-chart,equity-drawdown-chart}`, `components/risk/{risk-budget-meter,account-risk-panel}`, `components/playbook/{edge-evolution-chart,setup-intelligence-panel,error-cards-panel}`.
