...
- **AUDITORÍA FINAL PENDIENTE** (a petición del usuario antes de cerrar): revisar open items S0–S14, deuda técnica, todos los docs/v3 + código, qué quedó fuera/incompleto → reporte. Diferidos conocidos: migración real de rutas (/hoy,/operar…), transfer #31/SRS #45 en /aprendizaje, telemetría de ignorado del feed (S13), digest #28, E19 ImprovementScore snapshot (S14), régimen exógeno POST-4, S7b write-tools del chat, edge-decay verifier (OI-4). **Diferido:** transfer #31 + SRS #45 (resource-centric → /aprendizaje, no /playbook; backend S11 ya existe). Componentes reutilizables: `components/analytics/{r-distribution-chart,equity-drawdown-chart}`, `components/risk/{risk-budget-meter,account-risk-panel}`, `components/playbook/{edge-evolution-chart,setup-intelligence-panel,error-cards-panel}`.
