...
- ✅ **AUDITORÍA FINAL hecha** → `AUDIT_FINAL.md`. **TRACK DE CIERRE en curso** (el usuario: "no se cierra v3 hasta saldar la deuda"). Specs: `CLOSURE_DESIGN.md` + `CLOSURE_SPRINT_PLAN.md`. **Re-verificación contra código** (`CLOSURE_REVERIFY.md`): **la deuda es mucho menor de lo que el audit nivel-doc sugería** — A4 (sección rules ya existe), C1 (detectLosingStreak/Oversizing/Emotion ya existen), A2 (cola SRS ya existe), C2 (overconfidence-bias ya existe) están parcial/mayormente hechos.
  ...
  - **Deuda genuina restante:** **B1/B2 historización (E18/E19 snapshots → curvas longitudinales)** [alto valor] · **A2 panel transferencia #31 + cablear computeNextReview #45** [medio] · pulido A4-origen/C2-sesgos#40 [bajo]. **Reclasificado a v3.2 (no bloquea cierre):** A3 rutas, C3/C4/D1 incrementales, **E1 memoria 4 capas** (su invariante irreversible D9 YA está). Modo: re-verificar vs código antes de construir; PR por sprint. **Diferido:** transfer #31 + SRS #45 (resource-centric → /aprendizaje, no /playbook; backend S11 ya existe). Componentes reutilizables: `components/analytics/{r-distribution-chart,equity-drawdown-chart}`, `components/risk/{risk-budget-meter,account-risk-panel}`, `components/playbook/{edge-evolution-chart,setup-intelligence-panel,error-cards-panel}`.
