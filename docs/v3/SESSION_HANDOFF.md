...
- ✅ **S9 hecho** (risk engine puro, read-only, 1050/1050 vitest, smoke real OK). **Pendiente: push de `feat/v3-s9-risk-prop` + que el usuario mergee.** Decisión registrada (D9.3): S9 es señal/warn; el **bloqueo duro por budget + freeze agregado se cablea en S13** reusando rules/account-lock. **Sprint 10** (siguiente, según SPRINT_PLAN: Playbook intelligence E8 — edge decay #12, evolución #21, drift #32, A/B variantes #50) — leer el plan para el alcance. Deuda S1–S9: limpia.
