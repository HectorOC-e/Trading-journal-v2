...
- ✅ **AUDITORÍA FINAL hecha** → `AUDIT_FINAL.md`. **TRACK DE CIERRE en curso** (el usuario: "no se cierra v3 hasta saldar la deuda"). Specs: `CLOSURE_DESIGN.md` + `CLOSURE_SPRINT_PLAN.md`. **Re-verificación contra código** (`CLOSURE_REVERIFY.md`): **la deuda es mucho menor de lo que el audit nivel-doc sugería** — A4 (sección rules ya existe), C1 (detectLosingStreak/Oversizing/Emotion ya existen), A2 (cola SRS ya existe), C2 (overconfidence-bias ya existe) están parcial/mayormente hechos.
  ...
  - ✅ **v3.2 D1 — COACH WRITE-TOOL `propose_rule` MERGED #123, verificado BD real**: el coach PROPONE una regla protectora pre-trade (plantillas seguras: max_trades_per_day/cooldown_after_loss/max_risk_per_trade) creando una `RuleSuggestion` **pending** (insightId=null) que el usuario acepta/descarta en el UI de S5 — nunca auto-aplica (FREEZE §7). Sin migración (reusa E10). Auto-cableado en el loop agéntico (`COACH_TOOLS`/`coach-agent`). Smoke: propone→pending→accept→regla creada. ✅ **D1·b `propose_commitment` MERGED #126, verificado BD real**: el coach propone un compromiso verificable (limit_trades/no_revenge/respect_risk/stay_on_plan) que aterriza como `proposed` (inerte — la máquina solo evalúa `active`) hasta que el usuario acepta en `BehaviorLoopPanel` ("El coach te propone"). Sin migración (status es String). Smoke: propone→proposed→accept→active. **D1 completo (regla dura + compromiso blando).**
  ...
  - ✅ **v3.2 Memoria 4-capas VISIBLE (transparencia, ADR-003 §3) MERGED #127, verificado UI en preview**: `coach.patterns`/`coach.episodes` (read-only) + `CoachMemoryLayers` en el panel del coach — "Patrones (confirmados por tus datos + % confianza)" y "Momentos recordados (evidencia, por relevancia, con etiqueta de evento + fecha)". El trader VE las 4 capas. Sin migración.
  - **Siguiente v3.2:** A3 rutas reales (refactor IA grande, cosmético) · POST-1..7 (apuestas con disparador, la mayoría no activado). **4 ejes del compañero cognitivo vivos + visibles: recuerda (E1) · actúa (D1) · aprende (C3) · te busca (C4).**
