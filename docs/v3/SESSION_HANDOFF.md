...
- ✅ **S13 hecho + MERGED (#114)** — HOY feed: puro `domains/cognitive/today/feed.ts` (`assembleTodayFeed` #36 con suelo para críticas + `detectDailyAnomaly` #44) + `today-service` (agrega insights/compromisos/sugerencias/refuerzos/anomalía/budget) + router `today.feed` + `TodayFeed` en el dashboard (HOY). Verificado visual (6 insights priorizados + RiskBudgetMeter). 1118 vitest. **Siguiente: S14** (ÚLTIMO — ImprovementScore con drivers #41, régimen+rendimiento #33, coste de indisciplina temporal #49, onboarding día-1 #48 [ya hecho en S12d]). **Diferido de S12/S13:** migración real de rutas (/hoy,/operar…) + transfer #31/SRS #45 en /aprendizaje + telemetría de ignorado del feed + digest #28. **Diferido:** transfer #31 + SRS #45 (resource-centric → /aprendizaje, no /playbook; backend S11 ya existe). Componentes reutilizables: `components/analytics/{r-distribution-chart,equity-drawdown-chart}`, `components/risk/{risk-budget-meter,account-risk-panel}`, `components/playbook/{edge-evolution-chart,setup-intelligence-panel,error-cards-panel}`.
