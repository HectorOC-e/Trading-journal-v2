...
- ✅ **AUDITORÍA FINAL hecha** → `AUDIT_FINAL.md`. **TRACK DE CIERRE en curso** (el usuario: "no se cierra v3 hasta saldar la deuda"). Specs: `CLOSURE_DESIGN.md` + `CLOSURE_SPRINT_PLAN.md`. **Re-verificación contra código** (`CLOSURE_REVERIFY.md`): **la deuda es mucho menor de lo que el audit nivel-doc sugería** — A4 (sección rules ya existe), C1 (detectLosingStreak/Oversizing/Emotion ya existen), A2 (cola SRS ya existe), C2 (overconfidence-bias ya existe) están parcial/mayormente hechos.
  ...
  - **v3.2 EN CURSO:** plan `V32_MEMORY_PLAN.md` (E1 memoria 4 capas). ✅ **E1·a memoria EPISÓDICA (E13) MERGED #120, verificado BD real** — `memory_episodes` (pgvector) + salience puro + record/recall + productores (intervención/check-in rojo) + inyección en el contexto del coach ("Momentos recordados"). Decay a read-time. **Siguiente: E1·b** (MemoryPattern E14 + Memory Agent confirma con N≥3 episodios P6 + más productores) · **E1·c** (MemoryIdentity E15 estructurada + Improvement E16 lee E19 + assembler 4 capas D10).
