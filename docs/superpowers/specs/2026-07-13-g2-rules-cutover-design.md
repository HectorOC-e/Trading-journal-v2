...
- Añadir: `reorder`, `templates` (la galería `TEMPLATES` ya fusiona base + protección),
  `createFromTemplate` → crea en `rules`. (El router automations no tenía `duplicate`; no se añade — YAGNI.)
