...
> Diseño aprobado 2026-07-09. Reduce `docs/` de 145 archivos a 5 (3 vivos + 2 anexos).

---

## 1. Problema

`docs/` tiene 145 archivos. La cantidad no es el problema principal; el problema es que
...
Por composición, los 145 archivos son:
...
| Specs/plans de features (`superpowers/`) | 29 | regenerables por la skill de brainstorming |
...
141 archivos. Por directorio:

| Directorio | Archivos | Nota |
|---|---|---|
| `docs/v3/` | 84 | incluye `adr/` (absorbido en `ARCHITECTURE.md`) |
| `docs/superpowers/` | 29 | **aprobado borrar**; la skill de brainstorming los regenera |
...
2. **`docs: eliminar bitácoras de proceso consolidadas (141 archivos)`**
   Solo borrados, vía `git rm`.

Separarlos permite `git revert` del borrado sin perder la consolidación, y hace el primer diff
legible (si fuese un commit único, git mostraría los nuevos como renames de los borrados).

Después: `graphify update .` — el grafo tiene 466 nodos con `source_file` en `docs/`, apuntando a
140 archivos que dejarán de existir.
...
- [ ] `docs/ARCHITECTURE.md` conserva todos los IDs `FREEZE-*` del original. Comprobable con
      `grep -oE "FREEZE-(P|D|E|EV)[0-9]+"` sobre ambos, comparando conjuntos.
