...
export default function MonthlyReportPage() {
  ...
  const maxWeek = Math.max(1, ...r.weekTrend.map(w => Math.abs(w.pnl)))
  ...
  return (
    <div className="max-w-[900px] mx-auto pb-12">
      ...
      <h1 className="text-[22px] font-bold text-[var(--ink)]">{MONTHS[month - 1]} {year}</h1>
      <p className="text-sm text-[var(--ink-3)] mb-5">Review mensual · {r.kpis.trades} trades · moneda base {r.baseCurrency}</p>
      ...
      {/* Week trend */}
      <Card className="mb-5">
        <Eyebrow>Tendencia semana a semana</Eyebrow>
        {r.weekTrend.length === 0 ? (
          <p className="text-sm text-[var(--ink-3)]">Sin trades este mes.</p>
        ) : (
          <div className="flex items-end gap-3 h-28">
            {r.weekTrend.map(w => (
              <div key={w.week} className="flex-1 flex flex-col items-center justify-end h-full">
                <span className="text-[10px] num mb-1" style={{ color: w.pnl >= 0 ? "var(--win)" : "var(--loss)" }}>{money(w.pnl)}</span>
                <div className="w-full rounded-t" style={{
                  height: `${Math.max(4, (Math.abs(w.pnl) / maxWeek) * 80)}%`,
                  background: w.pnl >= 0 ? "var(--win)" : "var(--loss)",
                }} />
                <span className="text-[10px] text-[var(--ink-3)] mt-1">{w.week}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-3 mb-5">
        {/* Setups */}
        <Card>
          <Eyebrow>Setups del mes</Eyebrow>
      ...
      {/* Saved review: summary, themes, goals */}
      {r.saved && (
        <Card>
          <Eyebrow>Resumen del mes</Eyebrow>
          {r.saved.summary ? <p className="text-sm text-[var(--ink-2)] mb-3">{r.saved.summary}</p> : <p className="text-sm text-[var(--ink-3)] mb-3">Sin resumen escrito.</p>}
          <div className="flex flex-wrap gap-1.5">
            {r.saved.keyThemes.map(t => <span key={t} className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--accent-soft)] text-[var(--accent)]">{t}</span>)}
            {r.saved.goalsMet.map(g => <span key={g} className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--win-soft)] text-[var(--win)]">✓ {g}</span>)}
            {r.saved.goalsSet.filter(g => !r.saved!.goalsMet.includes(g)).map(g => <span key={g} className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--loss-soft)] text-[var(--loss)]">✗ {g}</span>)}
          </div>
        </Card>
      )}
    </div>
