...
import {
  TYPE_COLORS, MASTERY_LABELS, calcPreviewNextReview, fmtRelativeTime,
  type RevisarState, emptyRevisarState,
} from "../utils/review-helpers"

export { MASTERY_LABELS, calcPreviewNextReview, fmtRelativeTime }
            ...
            <>
              {([ ["🧠 ¿Qué aprendiste?", "learned", "Resume el concepto o lección más importante..."],
                ...
                <div key={key}>
                  ...
                  <textarea rows={3} className="w-full px-3 py-2 rounded-[var(--radius-sm)] text-sm bg-[var(--panel-2)] border border-[var(--line)] text-[var(--ink)] placeholder:text-[var(--ink-3)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)] resize-none"
                    placeholder={placeholder} value={form[key] as string} onChange={(e) => setField(key, e.target.value as RevisarState[typeof key])} />
                </div>
              ))}
              <div>
